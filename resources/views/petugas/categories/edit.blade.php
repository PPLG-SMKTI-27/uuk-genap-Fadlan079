        ...
        <div class="relative z-10 px-4 py-6 md:px-8 md:py-8 flex flex-col md:flex-row justify-between items-center gap-4">
            <div class="w-full md:w-2/3 space-y-2">
                <h2 class="text-2xl md:text-3xl font-extrabold text-white leading-tight tracking-tight">
                    Ubah Informasi<br>
                    <span class="text-secondary">Kategori: {{ $kategori->category_name }}</span>
                </h2>
...
<section class="p-5 pt-0">
    <div class="bg-white rounded-lg shadow-md border-t-[5px] border-primary max-w-2xl mx-auto">
        ...
        <div class="px-6 py-4 border-b border-gray-100 flex items-center gap-3">
            ...
            <div>
                <h3 class="font-bold text-text text-base">Formulir Pembaruan Kategori</h3>
            </div>
        </div>
        ...
        <form action="{{ route('kategori.update', $kategori->id) }}" method="POST" class="p-6 space-y-5">
            ...
            <div class="space-y-1.5">
                ...
                <textarea name="description"
                          ...
                          placeholder="Masukkan deskripsi singkat kategori...">{{ old('description', $kategori->description) }}</textarea>
                @error('description')
                    <p class="text-xs text-red-500 font-semibold">{{ $message }}</p>
                @enderror
            </div>
