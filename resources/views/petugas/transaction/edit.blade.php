        ...
        <div class="relative z-10 px-4 py-6 md:px-8 md:py-8 flex flex-col md:flex-row justify-between items-center gap-4">
            <div class="w-full md:w-2/3 space-y-2">
                ...
                <p class="text-gray-200 text-xs md:text-sm max-w-md leading-relaxed">
                    Pastikan data transaksi di isi dengan benar dan sesuai.
                </p>
            </div>
        </div>
    </div>
...
<section class="p-5 pt-0">
    <div class="bg-white rounded-lg shadow-md border-t-[5px] border-primary max-w-2xl mx-auto">
        ...
        <form action="{{ route('petugas.transaksi.update', $transaction->id)}}" method="POST" class="p-6 space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-1.5">
                ...
                <select name="status"
                    ...
                    @foreach(['pending', 'completed', 'cancelled'] as $t)
                        <option value="{{ $t }}"
                         {{ old('status', $transaction->status) == $t ? 'selected' : '' }}>
                            {{$t}}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-100">
                ...
                <button type="submit"
                    ...
                    <i class="fa-solid fa-floppy-disk text-xs"></i> Simpan Pembaruan
                </button>
            </div>
        </form>
